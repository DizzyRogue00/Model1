...
class Collaborative(object):
    ...
    def __Optimal(self,n,data=None,database=None,current_data=None):
        try:
            ...
            holding_time=m.addVars(range(2,self.N+1),name='holding_time')
            inter_holding_time=m.addVar(name='inter_holding_time')
            ...
            m.addConstrs((tau[j]==inter_tau_1[j]+holding_time[j] for j in range(2,self.N+1) if j!=math.ceil(self.N/2)+1),name='duration')
            m.addConstr(tau[math.ceil(self.N/2)+1]==inter_holding_time+holding_time[math.ceil(self.N/2)+1],name='inter_holding_time_con')
            #m.addConstr(tau[math.ceil(self.N/2)+1]==max_(inter_tau_1[math.ceil(self.N/2)+1],inter_tau_2)+holding_time[math.ceil(self.N/2)+1],name='duration_transship')
            m.addConstr(inter_holding_time== max_(inter_tau_1[math.ceil(self.N / 2) + 1], inter_tau_2), name='duration_transship')
            ...
            return self._data_record,self._objVal,self._result,self._departure, self._arrival, self._in_vehicle_j, self._in_vehicle, self._board, self._w, self._phi, self._tau, self._alight,self._holding_time
    ...
    def Analysis(self):
        ...
        data_result = pd.DataFrame(columns=["In-vehicle", "At-stop", "Extra", "Tardiness","Total"], index=["FTHC"])
        data_result.loc['FTHC']=result
        ...
        sheet_name="FTHC_Demand"+str(self.demand)
        if path.exists():
        ...
        plt.tight_layout()
        #plt.show()
        filename="Passengers Loads under Freight Transport under holding control when demand is "+str(self.demand)
        ...
        #plt.figure(num=19, facecolor='white', edgecolor='black')
        plt.figure(facecolor='white', edgecolor='black')
        plt.rcParams['font.family']='serif'
        plt.rcParams['font.serif']='Times New Roman'
        for i,df in data_b:
        ...
        plt.grid(False)
        filename="About to board and stranded passengers because of capacity limit under freight transport under holding control when demand is "+str(self.demand)
        filename_svg = self.combine_path(folder, filename, "svg")
        ...
        #plt.figure(num=20, facecolor='white', edgecolor='black')
        plt.figure(facecolor='white', edgecolor='black')
        plt.rcParams['font.family']='serif'
        plt.rcParams['font.serif']='Times New Roman'
        ...
        ax.legend(legend_label, loc='upper right', framealpha=0.5,
                       facecolor='white')
        plt.grid(False)
        filename="Average number of about to board and stranded passengers because of capacity limit under freight transport under holding control when demand is "+str(self.demand)
        filename_svg = self.combine_path(folder, filename, "svg")
        plt.savefig(filename_svg)
        filename_pdf = self.combine_path(folder, filename,"pdf")
        plt.savefig(filename_pdf, dpi=1000)
       ...
       #print(temp_list)
        ...
        #plt.figure(num=21, facecolor='white', edgecolor='black')
        plt.figure(facecolor='white', edgecolor='black')
        plt.rcParams['font.family']='serif'
        plt.rcParams['font.serif']='Times New Roman'
        ...
        plt.tight_layout()
        #plt.show()
        filename="Bus Trajectory under Freight Transport under holding control when demand is "+str(self.demand)
        filename_svg = self.combine_path(folder, filename, "svg")
        plt.savefig(filename_svg,bbox_extra_artists=(lg,),bbox_inches='tight')
        filename_pdf = self.combine_path(folder, filename, "pdf")
        plt.savefig(filename_pdf, dpi=1000,bbox_extra_artists=(lg,),bbox_inches='tight')
    ...
    def Average_Analysis(self):
        ...
        index_name="FTHC_Average_Demand_"+str(self.demand)
        ...
        plt.tight_layout()
        #plt.show()
        filename="Passengers Average Loads under Freight Transport under holding control when demand is "+str(self.demand)
        filename_svg=self.combine_path(folder,filename,"svg")
        #plt.savefig('Results/Average passenger arrival rates at each bus stations.svg')
        plt.savefig(filename_svg,bbox_extra_artists=(lg,),bbox_inches='tight')
        filename_pdf = self.combine_path(folder, filename, "pdf")
        ...
        colors = [plt.cm.Spectral(i / float(2 - 1)) for i in range(2)]
        #plt.figure(num=23,facecolor='white',edgecolor='black')
        plt.figure(facecolor='white', edgecolor='black')
        plt.rcParams['font.family'] = 'serif'
        plt.rcParams['font.serif'] = 'Times New Roman'
        ...
        plt.grid(False)
        filename = "Average Goods Load Rate under freight transport under holding control when demand is " + str(self.demand)
        filename_svg = self.combine_path(folder, filename, "svg")
        plt.savefig(filename_svg)
        filename_pdf = self.combine_path(folder, filename, "pdf")
        plt.savefig(filename_pdf,dpi=1000)
        ...
        colors = [plt.cm.Spectral(i / float(2 - 1)) for i in range(2)]
        #colors=['#8E354A','#261E47']
        #plt.figure(num=24, facecolor='white', edgecolor='black')
        plt.figure(facecolor='white', edgecolor='black')
        plt.rcParams['font.family']='serif'
        plt.rcParams['font.serif']='Times New Roman'
        for i,df in data_b:
        ...
        lg = ax.legend(legend_label,loc='upper right',framealpha=0.5,
                       facecolor='white', edgecolor='black')
        plt.grid(False)
        filename="About to board and stranded passengers because of capacity limit for several times under freight transport under holding control when demand is "+str(self.demand)
        filename_svg = self.combine_path(folder, filename, "svg")
        plt.savefig(filename_svg)
        filename_pdf = self.combine_path(folder, filename, "pdf")
        plt.savefig(filename_pdf,dpi=1000)
        ...
        colors=['#9F353A','#66327C']
        #print(data_b_average)
        #plt.figure(num=25, facecolor='white', edgecolor='black')
        plt.figure(facecolor='white', edgecolor='black')
        plt.rcParams['font.family']='serif'
        plt.rcParams['font.serif']='Times New Roman'
        plt.bar(data_b_average.index,data_b_average['Boarding'],width=bar_width,align='center',color=colors[0])
        ...
        ax.legend(legend_label, loc='upper right', framealpha=0.5,
                       facecolor='white')
        plt.grid(False)
        filename="Average number of about to board and stranded passengers because of capacity limit for several times under freight transport under holding control when demand is "+str(self.demand)
        filename_svg = self.combine_path(folder, filename, "svg")
        plt.savefig(filename_svg)
        filename_pdf = self.combine_path(folder, filename,"pdf")
        plt.savefig(filename_pdf, dpi=1000)